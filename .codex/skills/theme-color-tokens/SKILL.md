...
- 凡命中白天/黑夜主题色、亮暗模式颜色、`AppColorTokens`、`AppThemeTokens`、页面新增 `Color(...)`、颜色收口等场景，必须先执行本 skill，再进行代码修改。
- 如果命中了上述场景，但没有先按本 skill 的检查流程执行，则不得继续实现。
- 除非用户明确要求做临时原型，否则页面层禁止直接新增亮暗两套 `Color(...)`；必须优先收口到主题体系。
